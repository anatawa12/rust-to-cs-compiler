...
pub fn rust_type_to_cs<'db>(ty: &Type<'db>, db: &'db dyn HirDatabase) -> String {
    rust_type_to_cs_inner(ty, db, false)
}

/// Render a type reference (in a Slot<T> context, the T).
pub fn rust_type_to_cs_slot<'db>(ty: &Type<'db>, db: &'db dyn HirDatabase) -> String {
    rust_type_to_cs_inner(ty, db, false)
}

fn rust_type_to_cs_inner<'db>(ty: &Type<'db>, db: &'db dyn HirDatabase, in_slot: bool) -> String {
    ...
    if let Some((inner, _mutability)) = ty.as_reference() {
        return rust_type_to_cs_inner(&inner, db, in_slot);
    }

    // Raw pointer → Ref<T>
    if let Some((inner, _)) = ty.as_raw_ptr() {
        return format!("Ref<{}>", rust_type_to_cs_inner(&inner, db, false));
    }

    // Slice
    if let Some(inner) = ty.as_slice() {
        return format!("{}[]", rust_type_to_cs_inner(&inner, db, false));
    }
    ...
    if ty.is_tuple() {
        ...
        let parts: Vec<String> = fields.iter().map(|t| rust_type_to_cs_inner(t, db, false)).collect();
        return format!("({})", parts.join(", "));
    }

    // ADT (struct/enum/union)
    if let Some((adt, args)) = ty.as_adt_with_args() {
        let rust_adt_name = adt_rust_name(&adt, db);
        // Check std library mappings first
        if let Some(mapped) = map_std_type(&rust_adt_name, &args, db) {
            return mapped;
        }
        ...
        let type_args: Vec<String> = args
            ...
            .map(|a| rust_type_to_cs_inner(a, db, false))
            .filter(|s| s != "void")
            .collect();
...
pub fn builtin_to_cs(builtin: BuiltinType) -> String {
...
fn adt_rust_name(adt: &Adt, db: &dyn HirDatabase) -> String {
    match adt {
...
fn map_std_type<'db>(
    ...
    match rust_name {
        ...
        "Vec" => {
            ...
            Some(format!("System.Collections.Generic.List<{}>", rust_type_to_cs_inner(inner, db, false)))
        }
        "Box" => {
            let inner = args.first()?.as_ref()?;
            Some(rust_type_to_cs_inner(inner, db, false))
        }
        "Arc" | "Rc" | "Mutex" | "RwLock" => {
            let inner = args.first()?.as_ref()?;
            Some(rust_type_to_cs_inner(inner, db, false))
        ...
        "Option" => {
            let inner = args.first()?.as_ref()?;
            let t = rust_type_to_cs_inner(inner, db, false);
        ...
        "Result" => {
            ...
            Some(format!(
                "s_Result<{}, {}>",
                rust_type_to_cs_inner(ok, db, false),
                rust_type_to_cs_inner(err, db, false)
            ))
        }
        "HashMap" | "BTreeMap" | "IndexMap" | "AHashMap" => {
            ...
            Some(format!(
                ...
                rust_type_to_cs_inner(k, db, false),
                rust_type_to_cs_inner(v, db, false)
            ))
        }
        "HashSet" | "BTreeSet" | "IndexSet" => {
            ...
            Some(format!(
                ...
                rust_type_to_cs_inner(inner, db, false)
            ))
        }
